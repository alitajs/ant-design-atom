...
import { NoticeIconTabProps } from './interface';
...
const NoticeIcon: React.FC<NoticeIconProps> & {
  ...
  const getNotificationBox = (): React.ReactNode => {
    ...
    return (
      <Spin spinning={loading} delay={300}>
        <Tabs className="atom-tabs" onChange={onTabChange}>
          {panes}
        </Tabs>
      </Spin>
    );
  ...
  const noticeButtonClass = classNames(className, 'atom-notice-button');
  const notificationBox = getNotificationBox();
  const NoticeBellIcon = bell || <BellOutlined className="atom-icon" />;
  const trigger = (
    <span className={classNames(noticeButtonClass, { opened: visible })}>
      <Badge count={count} style={{ boxShadow: 'none' }} className="atom-badge">
        {NoticeBellIcon}
      </Badge>
    </span>
  );
